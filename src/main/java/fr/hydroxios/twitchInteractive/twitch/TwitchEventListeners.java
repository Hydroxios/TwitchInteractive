...
public class TwitchEventListeners {
    ...
    public static void onMessage(ChannelMessageEvent e){
        ...
        if(message.startsWith(TwitchCommandManager.PREFIX)){
            ...
            Optional<ITwitchCommand> twitchCommand = Twitch.get().getCommandManager().find((c) -> c.getName().equalsIgnoreCase(label));
            twitchCommand.ifPresent((c) -> c.execute(e.getUser(), args));
        }
    }

    public static void onFollow(FollowEvent e){
        Bukkit.broadcastMessage(References.PREFIX + ChatColor.translateAlternateColorCodes('&', "&b" + e.getUser().getName() + " &fis now following !"));
