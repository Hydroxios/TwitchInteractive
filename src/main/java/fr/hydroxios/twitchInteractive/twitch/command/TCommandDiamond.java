...
public class TCommandDiamond implements ITwitchCommand{
    ...
    public void execute(EventUser sender, String[] args) {
        if(TwitchInteractive.INSTANCE.getConfig().contains("main_player")){
            Player p = Bukkit.getPlayer(UUID.fromString(TwitchInteractive.INSTANCE.getConfig().getString("main_player")));
            if(p == null || !p.isOnline()) return;
            p.getInventory().addItem(new ItemStack(Material.DIAMOND));
            p.sendMessage(References.PREFIX + ChatColor.translateAlternateColorCodes('&', "&6" + sender.getName() + " &fgave you a &bdiamond&f !"));
        } else {
